...
| Unsafe automation | Manual is the default; native definition can be inspected; enable requires a matching untampered real-model editorial shadow with usable content and no model failures plus online preflight; a source-only preview cannot unlock scheduling; enable/disable requires confirmation; exact prior OS state is restored if state recording fails |
